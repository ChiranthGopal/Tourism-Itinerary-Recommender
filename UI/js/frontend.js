...
cur_usr_lat=12.29;
cur_usr_lng=76.64;
//this function calculates the no of days and time availble for user  to  visit the places around mysore.
function getTime()
{
...
// this funtion loads the map initially
function loadMap() 
{
...
//function to initialize the necessary variables. This function is executed when the HTML is loaded
function init()
{
...
//function  calls the another function (showplaces) after 2 seconds delay.
...
//functions to initilize the map
function initMap() 
{
...
//function to normalize ratings and distance .
...
//this function lists the places sent by server according to the rating and no of reviews
...
//function is callback from the server. This function lists places sent by server in tabular form. table is generated dynamically
function updateinfo()
{
...
waypoint=[];
//function calculates the minimum cost in the array of costs 
...
MAX = 99999;
//function to calculate the cost of all  POIs from source . source is the row no and row contents are destination.  
...
//this function decides which are the places are the most cost effective if included in itinerary per day.
function calculate_waypoints(regi,cost_matrix,wish_list)
{
	...
	visited ={};
	while_breaker=false;
	//this while loop checks if all the places of users choice is included in the itinerary or not.
	while(Object.keys(visited).length<Object.keys(wish_list).length)
	{
		...
		//this if checks if the time is available to visit the next POI or not.
...
function showPlaceNames()
{
    ...
    div.innerHTML = "<h2>Day "+day+"</h2></br>";
    ...
    dep.innerHTML = pad(start_time[0],2)+":"+pad(start_time[1],2);
    ...
    for(i=0;i<x.length;i++)
    {
        ...
        dep = document.createElement("td");
        visitDuration = document.createElement("td");
        arr.innerHTML = pad(current_time[0],2)+":"+pad(current_time[1],2);
        pla.innerHTML = x[i];
        ...
        dep.innerHTML = pad(current_time[0],2)+":"+pad(current_time[1],2);
        visitDuration.innerHTML = pad(v_times[i][0],2)+":"+pad(v_times[i][1],2);
        tr.appendChild(pla);
        tr.appendChild(arr);
        tr.appendChild(visitDuration);
        tr.appendChild(dep);
...
//this function  checks for all the places selected by user and sends them to further itinerary generator
function checkClick()
{
    ...
    //auto check some no of places()
    if(first_time==1)
    {
    	console.log(days);
        if(days==0 || remaining_time==0)
        {
            alert("PLEASE SELECT  DATE  OR TIME FOR TOUR");
        }
        else
        {
            lngth = days*5;
            lngth = Math.min(data.place.length,lngth);
            for(i=0;i<lngth;i++)
            {
                id="check"+data.place[i]['place_name'];
...
//function to select the particular day . this function gives days list as dropdown and user can choose any day of his wish.
...
//function generates the itinerary for each day of users visit. However map is showed in the website for only one day at a time 
...
// this function sends the categories selected to server for autorecommendation . callback function from server is updateinfo
function recommend()
{
    ...
    else if(first_time==1)
    {
        ...
        xhr.open("GET","http://localhost/Project/retrieve.php?category="+encodeURIComponent(toSend),true);
        xhr.send(); 
        console.log("toSend from recommend ",toSend);
    }
}  
